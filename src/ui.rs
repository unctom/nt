...
fn render_footer(f: &mut Frame, app: &App, area: Rect) {
    ...
    let binds = match app.mode {
        Mode::Normal => "q/Esc: Quit | j/k: Nav | a: Add | space: Toggle | x: Del | /: Search | g: Global",
...
mod tests {
    ...
    fn test_render_with_notes_normal_mode() {
        ...
        app.notes.push(Note::new("task 1".to_string(), "test".to_string()));
        app.notes.push(Note::new("task 2 #urgent".to_string(), "test".to_string()));
    ...
    fn test_render_modes() {
        let mut app = App::new("test".to_string(), vec![]);
        app.notes.push(Note::new("task".to_string(), "test".to_string()));
